...
cp /root/gitbind_id_rsa /root/.ssh/id_rsa
chmod 400 /root/.ssh/id_rsa

# "Add Git server to known_hosts"
echo "Add Git server to known_hosts"
ssh-keyscan -H git.cpg.org >> /root/.ssh/known_hosts

# Empty Puppet directory (to be populated with Git code)
rm -rf /etc/puppetlabs/code/environments/production/* &&

# Clone code
git clone git@git.cpg.org:devops/puppet.git /etc/puppetlabs/code/environments/production/
